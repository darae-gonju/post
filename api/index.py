...
def get_col_safe(df, name, n):
    ...
    for c in df.columns:
        if str(c).replace(" ", "") == target_clean:
            return df[c].fillna("").astype(str).apply(lambda x: str(x).strip())
    return pd.Series([""] * n)
...
def convert():
    try:
        ...
        # 4. 상세주소 필수값 에러 방지 (빈칸이면 공백 한 칸 추가)
        addr_detail = get_col_safe(df, "상세배송지", num_rows)
        addr_detail = addr_detail.replace("", " ")
        ...
        mapping_data = {
            ...
            "내용물": get_col_safe(df, "상품명", num_rows).apply(lambda x: str(x)[:20]),
            "배달방식": ["미신청"] * num_rows,
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f"post_office_converted.xlsx"
        )
